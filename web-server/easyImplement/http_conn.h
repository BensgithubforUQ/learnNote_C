...
private:
    ...
    bool process_write( HTTP_CODE ret );    // 填充HTTP应答 向m_write_buf写入响应报文数据
...
private:
    ...
    //定义了一个向量元素，通常，这个结构用作一个多元素的数组,该结构体中iov_base指向数据的地址,iov_len表示数据的长度
    int m_iv_count;                         //其中m_iv_count表示被写内存块的数量。
    int cgi;                        //是否启用的POST
    char *m_string;                 //存储请求头数据（消息体）
